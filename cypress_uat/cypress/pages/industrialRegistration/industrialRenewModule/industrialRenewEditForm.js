// step0: Applicant industrial renew registration
export const industrialRenewAddForm = () => {
    cy.get('.panel-body').eq(0).contains('a', 'আবেদন').click() // click on শিল্প নিবন্ধন card   
    cy.wait(5000);
    cy.contains('label.checkbox-inline', 'নবায়ন').first().click(); // click "renew" radio button
    cy.wait(2000);
...
// step1: Open Applicant industrial renew registration edit blade and submit for payment
export const industrialRenewEditSubmitFormOpen = () => {
    ...
    cy.url({ timeout: 10000 }).should('include', '/client/industry-renew/list/')
    cy.wait(3000)
...
export const viewBladeSectionsCheck = () => 
{
    ...
    // check texts exist - Section 1: শিল্প নবায়ন
    cy.contains('span.section_head', 'শিল্প নবায়ন').scrollIntoView().should('be.visible')
