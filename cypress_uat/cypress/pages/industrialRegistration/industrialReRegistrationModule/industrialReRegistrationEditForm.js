// step0: Applicant industrial re-registration
export const industrialReRegistrationAddForm  = () => {
    ...
    //1. save_as_draft tab click
    cy.get('#save_as_draft').click()
    cy.wait(5000)
}

// step1: Open Applicant industrial re-registration edit blade and submit for payment
export const industrialReRegistrationEditSubmitFormOpen = () => {
    ...
    cy.get('button[data-action*="local-raw-material"]').should('be.visible').first().click()
    cy.get('.modal:visible').last().within(() => {
        cy.get('#local_raw_material_manual_input').should('be.visible').click()
        cy.get('input[name="local_raw_material_name"]').should('be.visible').first().clear().type('A')
        cy.get('input[name="local_raw_material_quantity"]').should('be.visible').first().clear().type('50')
        cy.get('select[name="local_raw_material_unit"]').should('be.visible').first().select('2')
        cy.get('input[name="local_raw_material_amount_bdt"]').should('be.visible').first().clear().type('20')
        cy.get('#local_raw_material_save').should('be.visible').and('not.be.disabled').click()
    })
    cy.get('.close_modal').first().should('be.visible').click()
    cy.wait(3000)

    //খ. আমদানিকৃত/আমদানিতব্য
    cy.get('button[data-action*="imported-raw-material"]').should('be.visible').first().click()
    cy.get('.modal:visible').last().within(() => {
        cy.get('#imported_raw_material_manual_input').should('be.visible').click()
        cy.get('input[name="imported_raw_material_name"]').should('be.visible').first().clear().type('B')
        cy.get('input[name="imported_raw_material_quantity"]').should('be.visible').first().clear().type('60')
        cy.get('select[name="imported_raw_material_unit"]').should('be.visible').first().select('2')
        cy.get('input[name="imported_raw_material_amount_bdt"]').should('be.visible').first().clear().type('30')
        cy.get('#imported_raw_material_save').should('be.visible').and('not.be.disabled').click()
    })
    cy.get('.close_modal_imported').first().should('be.visible').click()
    cy.wait(3000)
    ...
    // cy.get('[id="addMoreDirector"]').first().should('be.visible').click()
    // cy.wait(2000);
    // cy.get('input[name="nationality_type"][value="bangladeshi"]').check().should('be.checked')
    // cy.wait(2000);
    // cy.get('input[name="identity_type"][value="nid"]').check().should('be.checked')
    // cy.wait(2000);
    // cy.get('[id="user_nid"]').type('5505028877')
    // cy.wait(2000)
    // cy.get('[id="nid_dob"]').should('be.visible').type('02-Jan-1985').click()
    // cy.wait(2000)
    // cy.get('[id="user_nid_name"]').type('MD JALAL SHEK')
    ...
    // cy.wait(2000)
    // cy.get('#NIDVerifyBtn').should('be.visible').click()
    // cy.wait(3000)
    // cy.get('[id="maleRadio"]').click({ force: true })
    // cy.wait(2000)
    // cy.get('[id="btn_save"]').click({ force: true })
    ...
    cy.get('[id="authorization_letter_edit"]').first().attachFile('1.pdf')
...
// step2: Process application as 9th grade officer to recommend to 6th grade or observation or shortfall
...
// step3: Process application as 6th grade officer to approve to applicant or reject or shortfall
