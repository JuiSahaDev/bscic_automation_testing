export const applicantLoginPage = () => {
    cy.contains(' Login ', { timeout: 20000 }).should('be.visible').click();
    cy.wait(2000);
    cy.contains('Login in Classic Mode', { timeout: 20000 }).should('be.visible').click();
    ...
    cy.origin('https://uat-id.oss.net.bd', () => {
        ...
        cy.get('[id="identifier"]', { timeout: 15000 }).should('be.visible').type('juisaha.ossp@gmail.com')
    ...
    cy.url({ timeout: 60000 }).should('include', 'uat.ossbscic.gov.bd')
    cy.wait(5000)
    
    // Handle company selection modal if it appears
    cy.get('body').then(($body) => {
        if ($body.find('button:contains("Submit"):visible').length > 0) {
            ...
            cy.log('No company selection modal - continuing')
        }
    })
}
...
export const deskUserLoginPage = () => {
    ...
    cy.url({ timeout: 10000 }).then((url) => {
        if (url.includes('uat-id.oss.net.bd')) {
            ...
            cy.wait(10000)
            cy.url({ timeout: 180000 }).should('include', 'uat.ossbscic.gov.bd')
            cy.wait(3000)
        } else if (url.includes('uat.ossbscic.gov.bd')) {
...
export const applicantLogoutPage = () => {
    cy.url().then((currentUrl) => {
        ...
        cy.get('li.dropdown.user-menu > a.dropdown-toggle', { timeout: 10000 }).should('be.visible').click();
        
        // Wait for dropdown to fully expand
        cy.wait(1000)
        
        // Wait for Sign out button to be visible and click it
        cy.get('a[href="https://uat.ossbscic.gov.bd/logout"]', { timeout: 10000 }).should('be.visible').then(($btn) => {
        ...
        // Visit homepage explicitly after logout to ensure clean state
