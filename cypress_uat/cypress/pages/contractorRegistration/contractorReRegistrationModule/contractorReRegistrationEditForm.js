// step0: Applicant contractor add re-registration form filling and save_as_draft
export const contractorReRegistrationAddForm = () => {
    ...
    // 1. সাধারণ তথ্য (Fill out the contractor registration form)
    cy.get('[id="org_type"]').should('be.visible').select('পার্টনারশীপ')
    cy.wait(2000);
    cy.get('[id="manual_reg_number"]').should('be.visible').type('CON-20250224-Manual', { force: true })
    cy.wait(2000);
    cy.get('#fiscal_year_start').select('2025', { force: true })
    cy.wait(2000);
    cy.get('[id="office_post_name"]').type('Dhaka', { force: true })
    cy.wait(2000);
    cy.get('[id="industry_adress_checkbox"]').click()
    cy.wait(2000);
    

    // প্রতিষ্ঠানের দায়িত্বপ্রাপ্ত ব্যক্তির পরিচয়
    cy.get('[id="addMoreDirector"]').first().should('be.visible').click()
    cy.wait(2000);
    cy.get('.modal:visible').last().within(() => {
        cy.get('input[name="director_name"][value="1"]').check({ force: true }).should('be.checked')
        cy.wait(2000)
        cy.get('input[name="nationality_type"][value="bangladeshi"]').check({ force: true }).should('be.checked')
        cy.wait(2000)
        cy.get('input[name="identity_type"][value="nid"]').check({ force: true }).should('be.checked')
        cy.wait(2000)
        cy.get('input[name="user_nid"]').first().clear().type('5505028877', { force: true })
        cy.wait(2000)
        cy.get('[id="nid_dob"]').first().clear().type('02-Jan-1985', { force: true }).click()
        cy.wait(2000)
        cy.get('[id="user_nid_name"]').first().clear().type('MD JALAL SHEK', { force: true })
        cy.wait(2000)
        cy.get('textarea[name="g-recaptcha-response"]').first()
            .invoke('val', 'test-token')
            .trigger('input', { force: true })
            .trigger('change', { force: true })
        cy.wait(2000)
        cy.get('button[name="nid_verify_btn"]:visible').first().scrollIntoView().click({ force: true })
        cy.wait(3000)
        cy.get('[id="maleRadio"]').first().check({ force: true })
        cy.wait(2000)
        cy.get('[id="nid_designation"]').first().should('be.visible').select('পরিচালক')
        cy.wait(2000)
        cy.get('[id="btn_save"]').first().click({ force: true })
    })
    cy.wait(3000)
    cy.get('[id="mother_name"]').type('Kakali Saha', { force: true }) // filling mother's name
    cy.wait(2000)
    cy.get('[id="company_ceo_fatherName"]').type('Joydeb Saha', { force: true }) // filling father's name
    cy.wait(2000)
    cy.get('[id="division"]').should('be.visible').select('ঢাকা')
    cy.wait(2000)
    cy.get('[id="district"]').should('be.visible').select('নারায়ণগঞ্জ')
    cy.wait(2000)
    cy.get('[id="thana"]').should('be.visible').select('নারায়নগঞ্জ সদর')
    cy.wait(2000)
    cy.get('[id="post_name"]').should('be.visible').type('Narayanganj Sadar', { force: true })
    cy.wait(2000)
    cy.get('[id="postcode"]').should('be.visible').type('1400', { force: true })
    cy.wait(2000)
    cy.get('[id="email"]').should('be.visible').type('jui@example.com', { force: true })
    cy.wait(2000)
    cy.get('[id="mobile"]').should('be.visible').type('1627363947', { force: true })
    cy.wait(2000)
    cy.get('[id="address"]').should('be.visible').type('Narayanganj Sadar, Narayanganj', { force: true })
    cy.wait(2000)

    // upload photo
    cy.get('[id="correspondent_photo"]').first().attachFile('photo.jpeg')
    cy.wait(2000)
    ...
    // প্রতিষ্ঠানের আর্থিক তথ্যাবলি
    cy.get('[id="bank_id"]').should('be.visible').select('Bangladesh Bank Limited') // ব্যাংকের নাম
    cy.get('[id="branch_id"]').should('be.visible').select('abc') // শাখার নাম
    cy.get('[id="bank_account_no"]').should('be.visible').type('01323456789', { force: true })
    cy.get('[id="bank_email"]').should('be.visible').type('jui@example.com', { force: true })
    cy.get('[id="bank_mobile_no"]').should('be.visible').type('1627363947', { force: true })
    cy.wait(2000)
...
// step0: Applicant contractor edit re-registration form filling and submission
export const contractorReRegistrationEditSubmitFormOpen = () => {
    ...
    //2.  সংযুক্তি
    // cy.get('[id="file2639"]').first().attachFile('1.pdf')
    cy.get('[id="accept_terms"]').scrollIntoView().click()
    cy.get('a.next').eq(0).click()
    cy.wait(5000)

    //3. পেমেন্ট এবং সাবমিট
    // cy.get('[id="save_as_draft"]').should('be.visible').should('be.enabled').click()
    ...
    cy.url({ timeout: 10000 }).should('include', '/client/contractor-re-registration/list/')
    cy.wait(3000)

    // Verify payment success message
    cy.get('.alert-success', { timeout: 15000 }).should('be.visible').and('contain', 'Payment submitted successfully')
...
export const chiefEngineerStep3 = () => 
{
    ...
    cy.get('#application_status', { timeout: 20000 }).should('be.visible').select('Approved')
    cy.wait(2000)
    cy.contains('button', 'Process').should('be.visible').click()
    cy.wait(3000)
...
// step4: Check certificate generation and application download
...
// step5: Check all sections in view blade
export const viewBladeSectionsCheck = () => 
{
    ...
    // check texts exist - Section 1: ঠিকাদার পুনঃনিবন্ধন
    cy.contains('span.section_head', 'ঠিকাদার পুনঃনিবন্ধন').scrollIntoView().should('be.visible')
