// step0: Applicant contractor add new registration form filling and save_as_draft
export const contractorNewAddForm = () => {
    cy.get('.panel-body').eq(1).contains('a', 'আবেদন').click()  // click on ঠিকাদার নিবন্ধন card  
    cy.wait(5000);
    cy.contains('label.checkbox-inline', 'নতুন').first().click(); // click "নতুন" radio button
    cy.wait(2000);
    cy.contains('a.btn', 'পরবর্তী').click(); // click next button
    cy.wait(2000);

    // 1. সাধারণ তথ্য (Fill out the contractor registration form)
    cy.get('[id="org_type"]').should('be.visible').select('পাবলিক লিমিটেড')
    cy.wait(2000);
...
export const contractorNewEditSubmitFormOpen = () => {
    ...
    cy.url({ timeout: 10000 }).should('include', '/client/contractor-new/list/')
    cy.wait(3000)
...
export const viewBladeSectionsCheck = () => 
{
    ...
    // check texts exist - Section 1: ঠিকাদার নিবন্ধন
    cy.contains('span.section_head', 'ঠিকাদার নিবন্ধন').scrollIntoView().should('be.visible')
