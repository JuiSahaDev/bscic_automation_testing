// step0: Applicant contractor amendment registration
export const contractorAmendmentAddForm = () => {
    ...
    cy.get('#org_nm_bn').should('be.visible').clear().should('have.value', '').type('জুই প্রাইভেট লিমিটেড')
    cy.get('#org_nm').should('be.visible').clear().should('have.value', '').type('Jui Private Limited')

    // প্রতিষ্ঠানের নিবন্ধিত ঠিকানা
    cy.get('#industryAddressEdit').should('be.visible').click()
    ...
    cy.wait(2000)
    cy.get('#office_address').should('be.visible').clear().should('have.value', '').type('Narayanganj Sadar, Narayanganj', { force: true })
    cy.wait(2000)
    // First Uncheck
    cy.get('#industry_adress_checkbox').should('be.checked').uncheck({ force: true });
    // Then Check Again
    cy.get('#industry_adress_checkbox').should('not.be.checked').check({ force: true });
...
// step0.1: Open Applicant contractor amendment registration edit blade and submit for payment
export const contractorAmendmentEditSubmitFormOpen = () => {
    ...
    cy.url({ timeout: 10000 }).should('include', '/client/contractor-amendment/list/')
    cy.wait(3000)
...
export const viewBladeSectionsCheck = () => 
{
    ...
    // check texts exist - Section 1: ঠিকাদার সংশোধন
    cy.contains('span.section_head', 'ঠিকাদার সংশোধন').scrollIntoView().should('be.visible')
